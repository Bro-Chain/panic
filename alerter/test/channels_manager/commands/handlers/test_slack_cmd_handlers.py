...
class TestSlackCommandHandlers(unittest.TestCase):
    def setUp(self) -> None:
        self.test_handler_name = 'test_slack_command_handlers'
    ...
    def test__str__returns_handler_name(self) -> None:
        self.assertEqual(self.test_handler_name,
                         str(self.test_slack_command_handlers))

    def test_handler_name_returns_handler_name(self) -> None:
        self.assertEqual(self.test_handler_name,
                         self.test_slack_command_handlers.handler_name)

    def test_logger_returns_handler_logger(self) -> None:
        self.assertEqual(self.dummy_logger,
                         self.test_slack_command_handlers.logger)

    def test_rabbitmq_returns_rabbitmq(self) -> None:
        self.assertEqual(self.test_rabbitmq,
                         self.test_slack_command_handlers.rabbitmq)

    def test_redis_returns_redis(self) -> None:
        self.assertEqual(self.test_redis,
                         self.test_slack_command_handlers.redis)

    def test_mongo_returns_mongo(self) -> None:
        self.assertEqual(self.test_mongo,
                         self.test_slack_command_handlers.mongo)

    def test_associated_chains_returns_associated_chains(self) -> None:
        self.assertDictEqual(
            self.test_associated_chains,
            self.test_slack_command_handlers.associated_chains)

    def test_slack_channel_returns_slack_channel(self) -> None:
        self.assertEqual(self.test_slack_channel,
                         self.test_slack_command_handlers.slack_channel)
    ...
    def test_redis_running_returns_true_if_redis_is_running(
        ...
        actual_ret = self.test_slack_command_handlers.redis_running()
    ...
    def test_redis_running_raises_exception_if_unexpected_err(
        ...
        self.assertRaises(Exception,
                          self.test_slack_command_handlers.redis_running)
    ...
    def test_mongo_running_returns_true_if_mongo_is_running(
        ...
        mock_ping_unsafe.return_value = None
        actual_ret = self.test_slack_command_handlers.mongo_running()
        self.assertTrue(actual_ret)
    ...
    def test_mongo_running_raises_exception_if_unexpected_err(
        ...
        mock_ping_unsafe.side_effect = Exception('Unexpected Exception')
        self.assertRaises(Exception,
                          self.test_slack_command_handlers.mongo_running)
    ...
    def test_rabbit_running_returns_true_if_rabbit_is_running(
        ...
        actual_ret = self.test_slack_command_handlers.rabbit_running()
        self.assertTrue(actual_ret)
    ...
    def test_rabbit_running_raises_exception_if_unexpected_err_when_connecting(
        ...
        mock_disconnect_unsafe.return_value = None
        self.assertRaises(Exception,
                          self.test_slack_command_handlers.rabbit_running)
    ...
    def test_ping_callback_does_not_return_PONG_if_unrecognized_user(
            self, mock_say, mock_authorise) -> None:
        mock_authorise.return_value = False
        mock_say.return_value = None
        self.test_slack_command_handlers.ping_callback(self.test_ack,
                                                       self.test_say,
                                                       self.test_command)
    ...
    def test_ping_callback_returns_PONG_if_recognized_user(
        ...
        self.test_slack_command_handlers.ping_callback(self.test_ack,
                                                       self.test_say,
                                                       self.test_command)
        mock_say.assert_called_once_with("PONG!")

    def test_get_running_icon_returns_tick_if_running(self) -> None:
        actual_ret = self.test_slack_command_handlers._get_running_icon(True)
        self.assertEqual(u'\U00002705', actual_ret)

    def test_get_running_icon_returns_cross_if_not_running(self) -> None:
        actual_ret = self.test_slack_command_handlers._get_running_icon(
            False)
    ...
    @mock.patch.object(SlackCommandHandlers, "mongo_running")
    def test_get_mongo_based_status_returns_correct_status_if_no_unrec_except(
        ...
        actual_status = \
            self.test_slack_command_handlers._get_mongo_based_status()
    ...
    @mock.patch.object(SlackCommandHandlers, "rabbit_running")
    def test_get_rabbit_based_status_returns_correct_status_if_no_unrec_except(
        ...
        actual_status = \
            self.test_slack_command_handlers._get_rabbit_based_status()
        self.assertEqual(expected_status_message, actual_status)
    ...
    def test_get_muted_status_returns_correct_status_if_no_severity_muted(
        ...
        actual_status = self.test_slack_command_handlers._get_muted_status()
        ...
        for permutation in chain1_severity_permutations:
            if ', '.join(permutation) in actual_status:
                expected_status += "- {} has {} alerts muted.\n".format(
                    self.test_chain_1, ', '.join(permutation))
                break

        expected_status += "- {} has no alerts muted.\n".format(
            self.test_chain_2)
        expected_status += "- {} has {} alerts muted.\n".format(
            self.test_chain_3, "INFO")
    ...
    def test_get_muted_status_return_correct_if_mute_and_muteall_severities(
        ...
        for permutation in chain3_severity_permutations:
            if ', '.join(permutation) in actual_status:
                expected_status += "- {} has {} alerts muted.\n".format(
                    self.test_chain_3, ', '.join(permutation))
                break
    ...
    def test_get_manager_component_hb_status_returns_empty_string_if_hb_ok(
        ...
        actual_ret = self.test_slack_command_handlers \
    ...
    def test_get_manager_component_hb_status_return_correct_if_missed_hbs(
        ...
        expected_ret = \
            ...
            "into problems. Please check the logs.\n".format(
                self.test_manager_component,
                self.test_slack_command_handlers._get_running_icon(False),
                expected_missed_hbs,
                self.test_manager_component)
    ...
    def test_get_panic_components_status_return_correct_if_problems_in_HC(
        ...
        expected_ret = "- *PANIC Components*: {} - Cannot get live status as " \
                       ...
                       "Checker.\n".format(self.test_slack_command_handlers
                                           ._get_running_icon(False))
        self.assertEqual(expected_ret, actual_ret)

    @mock.patch.object(SlackCommandHandlers,
    ...
    def test_get_panic_components_status_return_correct_if_all_ok(
        ...
        expected_ret = "- *PANIC Components*: {}\n".format(
            self.test_slack_command_handlers._get_running_icon(True))
        self.assertEqual(expected_ret, actual_ret)
        ...
        for component, hb_available in components_hb_available_dict.items():
            if not hb_available:
                expected_ret += \
                    "- *{}*: {} - No heartbeats yet.\n".format(
                        component,
                        self.test_slack_command_handlers._get_running_icon(
                            False))
        self.assertEqual(expected_ret, actual_ret)
    ...
    def test_get_health_checker_status_ret_correct_if_hc_sub_components_ok(
        ...
        expected_ret = ("- *Health Checker*: {}\n".format(
            self.test_slack_command_handlers._get_running_icon(True)), False)
        actual_ret = \
            self.test_slack_command_handlers._get_health_checker_status()
        self.assertEqual(expected_ret, actual_ret)
        actual_ret = \
            self.test_slack_command_handlers._get_health_checker_status()
        self.assertEqual(expected_ret, actual_ret)
    ...
    @mock.patch.object(SlackCommandHandlers, "_get_muted_status")
    @mock.patch.object(SlackCommandHandlers, "_get_health_checker_status")
    @mock.patch.object(SlackCommandHandlers, "_get_panic_components_status")
    def test_get_redis_based_status_ret_correct_if_redis_accessible(
        ...
        actual_ret = \
            self.test_slack_command_handlers._get_redis_based_status()
    ...
    def test_status_callback_does_not_return_status_if_user_not_authorised(
            self, mock_authorise, mock_say) -> None:
        # We will perform this test by checking that Say() is not called.
        mock_authorise.return_value = False
        self.test_slack_command_handlers.status_callback(
            self.test_ack, self.test_say, self.test_command)
        mock_say.assert_not_called()

    @mock.patch.object(Say, '__call__')
    @mock.patch.object(SlackCommandHandlers, "_get_mongo_based_status")
    @mock.patch.object(SlackCommandHandlers, "_get_rabbit_based_status")
    @mock.patch.object(SlackCommandHandlers, "_get_redis_based_status")
    @mock.patch.object(SlackCommandHandlers, "_authorise")
    def test_status_callback_sends_correct_messages_if_user_authorised(
            ...
            mock_get_mongo_status, mock_say) -> None:
        # We will perform this test by checking that
        # Say() is called with the correct params.
        mock_authorise.return_value = True
    ...
    @parameterized.expand([
        (RedisError('test'), "Redis error. Muting may not have been successful "
                             "for all chains. Please check /panicstatus or "
                             "the logs "
        ...
        (ConnectionResetError('test'), "Redis error. Muting may not have been "
                                       ...
                                       "check /panicstatus or the logs to see "
                                       "which "
        ...
        (Exception('test'), "Unrecognized error. Please debug the issue by "
                            ...
                            "successful for all chains. Please check "
                            "/panicstatus "
    ...
    def test_mute_callback_sends_correct_replies_if_error_when_muting(
            self, muting_error, expected_reply, mock_say,
    ...
    def test_unmute_callback_does_not_unmute_chains_if_user_unrecognised(
        ...
        self.test_slack_command_handlers.unmute_callback(self.test_ack,
                                                         self.test_say,
                                                         self.test_command)
    ...
    def test_unmute_callback_does_nothing_if_chains_not_muted(
            self, mock_say, mock_authorise, mock_hexists_unsafe,
    ...
    def test_unmute_callback_unmutes_muted_chains(
            self, muted_chains, mock_say, mock_authorise) -> None:
        # To make sure that there are no persistent keys from other tests.
        self.test_redis.delete_all_unsafe()
        mock_say.return_value = None
        mock_authorise.return_value = True
    ...
    def test_unmute_callback_sends_correct_replies_if_chains_not_muted(
            self, mock_say, mock_authorise, mock_hexists_unsafe) -> None:
        mock_say.return_value = None
        mock_authorise.return_value = True
        mock_hexists_unsafe.return_value = False
        chain_names = [chain_name
    ...
    def test_unmute_callback_sends_correct_replies_if_chains_muted(
        ...
        successfully_unmuted_chains = [
            self.test_associated_chains[eval(chain_id)]
            for chain_id in muted_chains
        ]
        all_chains = [self.test_chain_1,
                      self.test_chain_2,
                      self.test_chain_3]
    ...
    def test_unmute_callback_sends_correct_replies_if_error_when_unmuting(
            self, unmuting_error, expected_reply, mock_say,
            mock_authorise, mock_hexists_unsafe) -> None:
        mock_say.return_value = None
        mock_authorise.return_value = True
    ...
    def test_unmuteall_callback_does_not_unmute_all_if_user_unrecognised(
        ...
        self.test_slack_command_handlers.unmuteall_callback(self.test_ack,
                                                            self.test_say,
                                                            self.test_command)
    ...
    @parameterized.expand([
        (RedisError('test'), "It may be that not all chains were unmuted due "
                             "to a Redis error. Check /panicstatus or the "
                             "logs to "
        ...
        (ConnectionResetError('test'), "It may be that not all chains were "
                                       ...
                                       "/panicstatus or the logs to see if "
                                       "unmuting "
        ...
        (Exception('test'), "It may be that not all chains were unmuted due "
                            "to an unrecognized error. Check /panicstatus or "
                            "the "
    ...
    def test_help_callback_does_not_return_help_message_if_user_not_authorised(
            self, mock_say, mock_authorise) -> None:
        mock_authorise.return_value = False
        self.test_slack_command_handlers.help_callback(self.test_ack,
                                                       self.test_say,
                                                       self.test_command)
        mock_say.assert_not_called()
    ...
    def start_callback_does_not_reply_with_start_msg_if_user_not_authorised(
            self, mock_say, mock_authorise) -> None:
        mock_authorise.return_value = False
        self.test_slack_command_handlers.start_callback(self.test_ack,
                                                        self.test_say,
                                                        self.test_command)
        mock_say.assert_not_called()
    ...
    def start_callback_replies_with_start_message_if_user_is_authorised(
        ...
        self.test_slack_command_handlers.start_callback(self.test_ack,
                                                        self.test_say,
                                                        self.test_command)
        expected_reply = "Welcome to PANIC's Slack commands!\n" \
    ...
    def test_authorise_returns_true_if_message_chat_id_is_valid(
            self, mock_say, mock_send_message) -> None:
        # A message channel id is valid if it is equal to the slack bot's
        # associated channel id, or if the slack bot's channel id is None.
        # Therefore in this test we will test for both cases
        mock_say.return_value = None
        mock_send_message.return_value = None

        # When the message chat id equals the slack bot's channel id
        self.test_command = {'channel_id':
                                 self.test_slack_bot_api.bot_channel_id}
        ret = self.test_slack_command_handlers._authorise(
            self.test_ack, self.test_command)
        self.assertTrue(ret)

        # When the slack bot's channel id is None
        self.test_command = {'channel_id': 245}
        self.test_slack_bot_api._bot_channel_id = None
        ret = self.test_slack_command_handlers._authorise(
            self.test_ack, self.test_command)
        self.assertTrue(ret)
    ...
    def test_authorise_returns_false_if_user_is_not_authorised(
        ...
        mock_say.return_value = None
        mock_send_message.return_value = None
        ret = self.test_slack_command_handlers._authorise(self.test_ack,
                                                          self.test_command)
