...
from src.utils.constants.rabbitmq import (
    STORE_EXCHANGE, TOPIC, SUBSTRATE_NETWORK_STORE_INPUT_QUEUE_NAME,
...
class SubstrateNetworkStore(Store):
    ...
    def _initialise_rabbitmq(self) -> None:
        ...
        Declares a queue named `substrate_network_store_input_queue` and binds
        it to the store exchange with a routing key
        `transformed_data.network.substrate` meaning anything coming from the
        transformer with regard to a substrate network will be received here.
        ...
        self.rabbitmq.exchange_declare(exchange=STORE_EXCHANGE,
                                       ...
                                       internal=False)
        self.rabbitmq.queue_declare(SUBSTRATE_NETWORK_STORE_INPUT_QUEUE_NAME,
                                    passive=False, durable=True,
                                    exclusive=False, auto_delete=False)
        self.rabbitmq.queue_bind(
            queue=SUBSTRATE_NETWORK_STORE_INPUT_QUEUE_NAME,
    ...
    def _process_redis_websocket_result_store(self, data: Dict) -> None:
        ...
        self.logger.debug(
            "Saving %s state: _last_monitored=%s, _grandpa_stalled=%s, "
            "_public_prop_count=%s, _active_proposals=%s, "
            "_referendum_count=%s, referendums=%s",
            chain_name, meta_data['last_monitored'], metrics['grandpa_stalled'],
            metrics['public_prop_count'], metrics['active_proposals'],
            metrics['referendum_count'], metrics['referendums']
        )
        ...
        self.redis.hset_multiple(
            Keys.get_hash_parent(parent_id),
            {
                ...
                Keys.get_substrate_network_grandpa_stalled(parent_id):
                    str(metrics['grandpa_stalled']),
                Keys.get_substrate_network_public_prop_count(parent_id):
                    str(metrics['public_prop_count']),
                Keys.get_substrate_network_active_proposals(parent_id):
                    json.dumps(metrics['active_proposals']),
                Keys.get_substrate_network_referendum_count(parent_id):
                    str(metrics['referendum_count']),
                Keys.get_substrate_network_referendums(parent_id):
                    json.dumps(metrics['referendums']),
            })
